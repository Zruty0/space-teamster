...
export function drawStartMenu(
  ...
  const rows = [
    ...
    { label: 'Teamster Operations Handbook (TOH)', detail: 'Read flight controls, procedures, and other help articles.' },
  ];
  ...
  ctx.textAlign = 'center';
  ctx.fillText(returnAvailable ? '↑↓: Select  Enter: Select  Esc: Return  |  1-5: Select' : '↑↓: Select  Enter: Select  |  1-5: Select', W / 2, startY + rows.length * lineH + 40);
...
export function drawHUD(
  ...
  if (state === 'flying') {
    ...
    const controls = ship.gearDeployed
      ? 'GEAR DOWN: W Full / S Idle  A/D 5% Lateral  SHIFT+A/D Max Lateral  Q/E Rotate  SPACE Hover  G Gear  T SAS  BACKSPACE Restart'
